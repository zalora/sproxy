...
When an HTTP client makes a request, sproxy checks for a `gauth` cookie.  If it
...
in a cookie: signed with a hash to prevent tampering, set for HTTP only (to
...
From that point on, when sproxy detects a valid `gauth` cookie it extracts the
